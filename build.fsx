...
let project = "LogApplication"
let summary = "Sample of stateful and stateless Service Fabric services with Suave"

let applicationName = "fabric:/LogApplication"
...
let gitOwner = "deepcovelabs"
let gitHome = "https://github.com/" + gitOwner
let gitName = "ServiceFabricSuaveStateful"
...
let pkgDir = "temp" </> "pkg"
let hostPkgDir = "src" </> "LogApplication" </> "pkg" </> "Release"
...
Target "RunIntegrationTest" (fun _ ->
    let errorCode =
        !! "temp/integrationTest/*IntegrationTests.exe"
        |> Seq.map (fun p -> shellExec {defaultParams with Program = p})
        |> Seq.sum
    if errorCode <> 0 then failwith "Error in tests"
)
...
let remove runspace =
    ...
    let applicationType = "BankValidationType" // this can be read from ApplicationManifest.xml
...
Target "DeployRemote" (fun _ ->
    let host =
        match getBuildParam "remote-host" with
        | s when not (String.IsNullOrWhiteSpace s) -> s
        | _ -> getUserInput "Remote Host: "
    "Connect-ServiceFabricCluster"
    |> psCommand
    |> psAddParameter ("ConnectionEndpoint", (sprintf "%s:19000" host) )
    |> invoke modules
    |> deploy)
